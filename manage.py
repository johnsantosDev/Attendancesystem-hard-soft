...
def show_studentDetails(serialdata,iD, scrf, x, title, now):
	...
	if serialdata==True :
		...
		#try :
		squery = 'SELECT * FROM LIST WHERE studentID=? OR studentName=? OR Class=?'
		n = (iD,iD,iD)
		cursor.execute(squery, n)
		lst = cursor.fetchall()
		if len(lst)>0:
			total_rows = len(lst)
...
def ReadfromArduino(port, scrf, arduino, iDEnt, progb, root,x, title, inA,inB,outA,outB):
	...
	def count():
		...
		if splitnow[0]==splitout[0] and splitnow[1]==splitout[1]:
			...
			com_ = str("COM"+port)
			link.Application(root, com_, inA,inB,outA,outB)
			print("-----------------------OUT TIME --------------------------------")
			root.after(10000)
			presences.clear()
...
class Application:
	...
	def __init__(self, root, com, iA,iB, oA,oB):
		#parameters
		self.COM = "COM"+com
		...
		ent1.place(x=20,y=5)
		ent1.insert(0, "Search for Student")
		tools.BindedEntry2(ent1,"Search for Studet")
		...
		update = ttk.Button(self.newcard, text="Update", 
			command=lambda:self.update(self.Namentry.get(), self.iDentry.get(),
						 self.Sexentry.get(), self.Classentry.get(), self.scx-80))
		update.place(x=470,y=65)
	...
	def update(self, name, iD, gender, class_,x):

		"""This function updates custom students data in a the STUDENTLIST database"""
		...
		cursor = connexion.cursor()
		#try :
		id_ = ('SELECT * FROM LIST WHERE studentID=?')
		cursor.execute(id_, [iD])
		studentFound = cursor.fetchall()
		if studentFound:
			action = '''UPDATE LIST SET studentName=?,Class=?,Gender=? WHERE studentID=?'''
			updates = (name, class_,gender,iD)
			cur = connexion.cursor()
			cur.execute(action, updates)
			connexion.commit()
			n = connexion.total_changes
			srch = ("SELECT * FROM LIST WHERE studentID=?")
			cursor.execute(srch, [iD,])
			lst = cursor.fetchall()
			if len(lst)>0:
				total_rows = len(lst)
				total_columns = len(lst[0]) 
			...
			messagebox.showinfo("Successfully", 
				f"data updated for student {name} with id {iD}, \n number of updates : {n}")	
		else:
			messagebox.showerror("DB ERROR", 
				"database is empty")
		#except:
